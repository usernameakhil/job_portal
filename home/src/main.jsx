...
function ApEmploymentPortal() {
  ...
  useEffect(() => {
    fetch('https://job-portal-backend-68x8.onrender.com/api/v1/management/public-aggregates')
      .then(res => res.json())
      .then(data => {
  ...
  // Structural Route Handler: Distributes connections straight to active live deployment anchors
  const navigateToProductionTenant = (targetUrl) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between selection:bg-emerald-800 selection:text-white font-sans">
      
      {/* 1. STATE EXECUTIVE IDENTITY NAVIGATION HEADER */}
      <header className="bg-white/90 border-b border-slate-200/80 sticky top-0 z-50 px-4 sm:px-8 py-3.5 flex flex-col lg:flex-row justify-between items-center gap-4">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          >
            ...
            <option value="Telugu">భాష: తెలుగు</option>
          </select>
        </div>
      </header>
      ...
      <main className="max-w-7xl w-full mx-auto px-4 sm:px-8 lg:px-10 pb-16 flex-grow flex flex-col justify-center">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl relative overflow-hidden flex flex-col justify-between shadow-sm hover:shadow-md transition-all duration-200">
